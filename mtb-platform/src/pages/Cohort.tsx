...
export default function Cohort() {
	...
	useEffect(() => {
		fetch(`${import.meta.env.BASE_URL}data/cohort.json`)
			.then((r) => r.json())
			.then(setD)
			.catch(() => setD(null));
	}, []);
	useEffect(() => {
		loadWorklist()
			.then((rows) =>
				Promise.all(rows.map((r) => loadReport(r.chartNo).catch(() => null))),
			)
	...
	return (
		<div className="gl-page" style={{ maxWidth: 1080 }}>
			...
			>
				...
				<GlCard header="Recurrent oncogenic drivers">
					{d.topGenes.map((g) => (
						...
						>
							<span className="mono gl-strong" style={{ width: 64 }}>
								{g.gene}
							</span>
							<span className="gl-funnel-track" style={{ height: 20 }}>
								<span
									className="gl-funnel-bar"
									style={{ width: `${(g.n / maxGene) * 100}%` }}
								>
									{g.n}
								</span>
							</span>
				...
				<GlCard header="Cancer types">
					{d.distribution.map((x) => (
						<div
							...
							style={{ gap: 10, marginBottom: 8 }}
						>
							<span className="gl-text-sm" style={{ width: 100 }}>
								{x.type}
							</span>
							<span className="gl-funnel-track" style={{ height: 20 }}>
								<span
			...
			{reports.length > 0 && (
				<div style={{ marginTop: 16 }}>
					<GlCard header="Sample matrix (oncoprint)">
						...
						>
							Recurrent alterations across the cohort, grouped by cancer type.
							Inspired by the ASH HematOmics Program (ASHOP) sample matrix.
						</p>
			...
			<div style={{ marginTop: 16 }}>
				<GlCard>
					<p className="gl-text-sm" style={{ margin: 0, lineHeight: 1.6 }}>
						{d.conclusion}
					</p>
					<div style={{ marginTop: 12, display: "flex", gap: 8 }}>
						<button
							...
							onClick={() => navigate("/worklist")}
						>
							Browse patient reports →
						</button>
						<button className="gl-button" onClick={() => navigate("/evidence")}>
		...
		<div style={{ marginTop: 16 }}>
			<GlCard header="Mutation mapper (lollipop)">
				<p className="gl-text-xs gl-text-muted" style={{ margin: "0 0 12px" }}>
					Cohort-wide protein map of point mutations per gene — recurrent
					residues cluster into hotspots against Pfam domains. In the spirit of
					the cBioPortal Mutation Mapper.
				</p>
				<div className="gl-row gl-wrap" style={{ gap: 6, marginBottom: 14 }}>
					{genes.map((g) => (
						<button
							...
							style={{ cursor: "pointer" }}
						>
							<span className="mono">{g.gene}</span>{" "}
		...
		<div style={{ marginBottom: 12 }}>
			<div className="gl-row gl-center gl-between" style={{ marginBottom: 4 }}>
				...
				<span className="gl-text-sm gl-text-muted">
					<span className="gl-strong" style={{ color: "var(--text)" }}>
						{n}
					</span>
					/{total} · {pct}%
				</span>
