	...
	if (error) {
		return (
			<div className="gl-page">
				...
				<GlLinkButton onClick={() => navigate("/worklist")}>
					← Back to worklist
				</GlLinkButton>
			</div>
		);
	...
	return (
		<div className="gl-page">
			...
			<div className="gl-row gl-center gl-wrap" style={{ gap: 10 }}>
				...
				<GlBadge variant={signed ? "success" : status.variant}>
					{signed ? "Signed off" : status.label}
				</GlBadge>
				<button
					className="gl-button"
					style={{ marginLeft: "auto" }}
					onClick={() => navigate(`/deck/${patient.chartNo}`)}
				>
					▷ Present case
				</button>
			</div>
			<div className="gl-meta">
				{patient.chartNo === "live" ? (
					...
					<>
						...
						<span className="dot">·</span>
						<span>
							{patient.sex === "F" ? "Female" : "Male"}, {patient.age}
						</span>
						<span className="dot">·</span>
						<span style={{ color: "var(--text)" }}>{patient.cancerType}</span>
						...
						<span className="dot">·</span>
						<span>
							{patient.team} · {patient.attending}
						</span>
					</>
				)}
			</div>
          ...
          its own vs. what genuinely needs the full pipeline. Nothing is faked. */}
			...
			{report.reannotation.events.length > 0 && (
				...
				>
					<div className="gl-card-body">
						...
						>
							<span
								style={{ color: "var(--orange-700)", display: "inline-flex" }}
							>
								<SyncIcon size={16} />
							</span>
							<span className="gl-strong">
								{report.reannotation.events.length} variant
								{report.reannotation.events.length > 1 ? "s" : ""} reclassified
								since this report was issued
							</span>
							<span className="gl-text-xs gl-text-muted">
								· re-annotated {report.reannotation.lastRun} ·{" "}
								{report.reannotation.knowledgeBase}
							</span>
						</div>
						<div className="gl-col" style={{ gap: 8 }}>
							{report.reannotation.events.map((e, i) => (
								...
								>
									{e.nowActionable && (
										<GlBadge variant="warning">Now actionable</GlBadge>
									)}
									...
									<GlBadge variant={ESCAT_META[e.fromTier].variant}>
										{ESCAT_META[e.fromTier].short}
									</GlBadge>
									<span className="gl-text-muted">→</span>
									<GlBadge variant={ESCAT_META[e.toTier].variant}>
										{ESCAT_META[e.toTier].short}
									</GlBadge>
								</div>
							))}
						</div>
						...
						>
							{report.reannotation.events[0].note}
						</p>
					</div>
				</div>
			)}
					...
					<div className="gl-card-body">
						{signed ? (
							<div className="gl-row gl-center" style={{ gap: 10 }}>
								<span
									className="gl-timeline-badge success"
								...
								<div>
									...
									<div className="gl-text-xs gl-text-muted">
										{selected.size} finding(s) taken to the board —{" "}
										{[...selected].join(", ")}
									</div>
								</div>
							</div>
							...
							<>
								...
								>
									Select actionable findings to take to the board, then sign
									off.
								</div>
								<div
									className="gl-row gl-center gl-between gl-wrap"
									style={{ gap: 12 }}
								>
										...
										{actionable.map((v) => {
											...
											return (
												...
												>
													...
													<GlBadge variant={ESCAT_META[v.escat].variant}>
									...
									<div className="gl-row" style={{ gap: 8 }}>
										<button
											className="gl-button"
											onClick={() => navigate(`/board/${patient.chartNo}`)}
										>
			...
			>
				<div>
					...
					{tab === "overview" && (
						<Overview report={report} onGoto={(t) => setTab(t as TabKey)} />
					)}
				...
				<aside className="gl-card" style={{ position: "sticky", top: 20 }}>
					<div className="gl-card-body">
						...
						<Glance label="Actionable (Tier I–II)">
							<GlCount
								value={actionable.length}
								active={actionable.length > 0}
							/>
						</Glance>
						<Glance label="Annotated alterations">
							<span className="gl-strong">{report.variants.length}</span>
						</Glance>
						<Glance label="Matched therapies">
							<span className="gl-strong">{counts.therapies}</span>
						</Glance>
						...
						<div className="gl-row gl-wrap" style={{ gap: 6 }}>
							<GlBadge
								variant={
									biomarkers.tmbClass === "TMB-High" ? "success" : "neutral"
								}
							>
								TMB {biomarkers.tmb}
							</GlBadge>
							<GlBadge
								variant={biomarkers.msi === "MSI-H" ? "success" : "neutral"}
							>
								{biomarkers.msi}
							</GlBadge>
							<GlBadge
								variant={
									biomarkers.hrdStatus.toLowerCase().includes("positive")
										? "success"
										: "neutral"
								}
							>
								{biomarkers.hrdStatus}
							</GlBadge>
						</div>
						<hr className="gl-divider" />
						<div
						...
						{report.reannotation.lastRun && (
							<div
								...
								style={{ marginTop: 2 }}
							>
								last {report.reannotation.lastRun} · next{" "}
								{report.reannotation.nextRun}
							</div>
						)}
						<hr className="gl-divider" />
						<div className="gl-text-xs gl-text-muted">Sample</div>
						<div className="mono gl-text-xs">{patient.sampleId}</div>
						<div className="gl-text-xs gl-text-muted" style={{ marginTop: 4 }}>
							{patient.panel}
						</div>
					</div>
				</aside>
...
function LiveProvenance({ report }: { report: Report }) {
	const actionable = report.variants.filter(
		(v) => v.escat === "I" || v.escat === "II",
	).length;
	const established = [
		...
		<div className="gl-card" style={{ marginTop: 16 }}>
			<div className="gl-card-body">
				<div className="gl-row gl-center" style={{ gap: 8, marginBottom: 4 }}>
					<span className="gl-strong">
						Provenance — this is a live VCF screen
					</span>
					<GlBadge variant="info">no server upload</GlBadge>
				</div>
				<p className="gl-text-xs gl-text-muted" style={{ margin: "0 0 12px" }}>
					Everything below is computed from the variant calls in your browser.
					Nothing that needs the full pipeline is inferred or faked.
				</p>
				<div
					className="gl-provenance-grid"
					style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}
				>
					<div>
						<div
							className="gl-text-xs gl-strong"
							style={{ color: "var(--green-600, #217645)", marginBottom: 6 }}
						>
					...
					<div>
						<div
							className="gl-text-xs gl-strong"
							style={{ color: "var(--orange-700, #a35200)", marginBottom: 6 }}
						>
							⋯ Needs the full pipeline to confirm
						</div>
						<ul
							className="gl-text-sm gl-text-muted"
							style={{ margin: 0, paddingLeft: 18, lineHeight: 1.6 }}
						>
							{needsPipeline.map((t) => (
								<li key={t}>{t}</li>
