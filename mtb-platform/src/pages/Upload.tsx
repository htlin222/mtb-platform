...
export default function Upload() {
	...
	const ingest = useCallback((text: string, name: string) => {
		try {
			...
			if (pr.total === 0) {
				setErr("No PASS variant records found — is this a VCF?");
				return;
			}
			setErr(null);
			setParsed(pr);
			logAudit({
				...
				detail: {
					...
					total: pr.total,
					somatic: pr.somatic,
					tierI_II: pr.actionable.length,
				},
				patient: name,
				fingerprint: fingerprint(text),
			});
		} catch {
			setErr("Could not parse this file as a VCF.");
		}
	}, []);
	...
	const useSample = () => {
		fetch(`${import.meta.env.BASE_URL}sample.vcf`)
			.then((r) => r.text())
			.then((t) => ingest(t, "sample.vcf"));
	};
	...
	const run = () => {
		...
		logAudit({
			trust: "human",
			op: "pipeline.run",
			summary: `Ran tertiary analysis on ${parsed.filename}`,
		...
		logAudit({
			...
			detail: {
				annotated: report.variants.length,
				tmbProxy: report.biomarkers.tmb,
				droppedVus: report.droppedVus,
			},
			patient: parsed.filename,
	...
	return (
		<div className="gl-page" style={{ maxWidth: 980 }}>
			...
			<div className="gl-page-title">
				<UploadIcon size={20} />
				<h1>New molecular analysis</h1>
			</div>
			<p className="gl-page-desc">
				Drop a tumour VCF — it is parsed in your browser, no upload to a server
				— then run it through the TSO500 tertiary-analysis pipeline. Or start
				from a demo case.
			</p>
			...
			>
				...
				<div className="gl-text-sm gl-text-muted" style={{ marginTop: 6 }}>
					...
					{" · "}
					<button className="gl-link-button" onClick={useSample}>
						use a sample VCF
					</button>
				</div>
			</div>
			...
			{err && (
				<div
					className="gl-card"
					style={{ borderColor: "var(--red-100)", marginBottom: 20 }}
				>
			...
			{parsed && (
				<div style={{ marginBottom: 28 }}>
					<GlCard
						header={
							<>
								Parsed <span className="mono">{parsed.filename}</span> — in your
								browser
							</>
						}
					>
						<div
							className="gl-row gl-wrap"
							style={{ gap: 24, marginBottom: 16 }}
						>
							<Stat n={parsed.total} label="PASS variants" />
							<Stat n={parsed.somatic} label="Somatic" />
							<Stat
								n={parsed.variants.filter((v) => v.gene).length}
								label="In actionable genes"
							/>
							<Stat
								n={parsed.actionable.length}
								label="Tier I–II"
								color="var(--blue-600)"
							/>
							<div style={{ alignSelf: "center" }}>
								<GlBadge variant="neutral">{parsed.reference}</GlBadge>
							</div>
						</div>
						...
						{parsed.variants.filter((v) => v.gene).length > 0 ? (
										...
										{parsed.variants
											...
											.map((v, i) => (
												<tr key={i}>
													...
													<td className="mono gl-text-sm">
														{v.annKind === "hotspot" ? v.alteration : "—"}
													</td>
													<td className="mono gl-text-xs gl-text-muted">
														{v.chrom}:{v.pos} {v.ref}&gt;{v.alt}
													</td>
													<td className="mono gl-text-sm">
														{v.af != null ? v.af.toFixed(2) : "—"}
													</td>
													<td>
														<GlBadge
															variant={
																v.annKind === "hotspot" ? "info" : "neutral"
															}
														>
															{v.annKind === "hotspot"
																? "Known hotspot"
													...
													<td>
														{v.tier ? (
															<GlBadge variant={ESCAT_META[v.tier].variant}>
																{ESCAT_META[v.tier].short}
															</GlBadge>
														) : (
															"—"
														)}
							...
							<p className="gl-text-sm gl-text-muted" style={{ margin: 0 }}>
								No variants fell in the bundled actionable-gene set. The full
								pipeline (VEP) annotates the complete gene set.
							</p>
						)}
						...
						>
							...
							<span className="gl-text-xs gl-text-muted">
								Gene tags are region/hotspot matches against a bundled hg19 set
								— the full run adds VEP + OncoKB + ESCAT + literature.
							</span>
						</div>
					</GlCard>
				</div>
			...
			>
				{(rows ?? []).map((r) => (
					<GlCard key={r.chartNo}>
						<div
							className="gl-row gl-center gl-between"
							style={{ marginBottom: 6 }}
						>
							<span className="gl-strong">{r.cancerType}</span>
							<GlBadge variant={r.actionableCount > 0 ? "info" : "neutral"}>
								{r.actionableCount} actionable
							</GlBadge>
						</div>
						<div
							className="gl-text-xs gl-text-muted mono"
							style={{ marginBottom: 4 }}
						>
							{r.sampleId}
						</div>
						<div className="gl-text-xs gl-text-muted">
							{r.stage} · {r.topFindings}
						</div>
						<div style={{ marginTop: 12 }}>
							<button
								className="gl-button gl-button-confirm"
								onClick={() => navigate(`/process/${r.chartNo}`)}
							>
