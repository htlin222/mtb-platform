...
export default function Batch() {
	...
	useEffect(() => {
		loadWorklist()
			.then((r) => {
				setRows(r);
				setSelected(new Set(r.map((x) => x.chartNo)));
			})
			.catch(() => setRows([]));
	}, []);
	...
	async function run() {
		...
		const reports = await Promise.all(
			chosen.map((r) => loadReport(r.chartNo).catch(() => null)),
		);
		const built: Track[] = [];
	...
	const legend = useMemo(
		() => [
			{ c: "#108548", label: "Tier I" },
			{ c: "#1f75cb", label: "Tier II" },
			{ c: "#c17d10", label: "Tier III" },
			{ c: "#bfbfc3", label: "Tier X" },
		],
		[],
	);
	...
	return (
		<div className="gl-page" style={{ maxWidth: 1100 }}>
			...
			<div className="gl-page-title">
				<ServerIcon size={20} />
				<h1>Batch analysis &amp; genome view</h1>
			</div>
			<p className="gl-page-desc">
				Select a batch of sequenced samples and visualise their actionable
				variants together on the GRCh37 genome (IGV.js), coloured by ESCAT tier.
			</p>
			...
			{rows && (
				<GlCard style={{ marginTop: 16 }}>
					...
					>
						<div className="gl-section-title" style={{ margin: 0 }}>
					...
					>
						{rows.map((r) => {
							...
							return (
								<div
									key={r.chartNo}
									className={`gl-picker-card${on ? " on" : ""}`}
									onClick={() => toggle(r.chartNo)}
								>
									<div className="gl-strong gl-text-sm">{r.name}</div>
									<div className="gl-text-xs gl-text-muted mono">
										{r.sampleId}
									</div>
									<div className="gl-text-xs gl-text-muted">
										{r.cancerType} · {r.actionableCount} actionable
									</div>
								</div>
							);
						})}
				...
				<div style={{ marginTop: 16 }}>
					...
					>
						<div className="gl-text-sm gl-text-muted">
							{totals.samples} samples · {totals.features} plotted variants ·{" "}
							{totals.actionable} Tier I–II
						</div>
						<div className="gl-batch-legend">
							{legend.map((l) => (
								<span key={l.label} className="gl-legend-dot">
					...
					<p className="gl-text-xs gl-text-muted" style={{ marginTop: 8 }}>
						One track per sample. Variants placed at their hg19 loci (exact
						hotspot position where known, else gene midpoint). Reference
						sequence and cytobands stream from the IGV genome service.
					</p>
				</div>
			)}
		</div>
